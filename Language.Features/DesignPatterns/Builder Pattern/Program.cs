...
namespace Builder_Pattern
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var dailyReportBuilder = new DailyReportBuilder(items);

            var reportDirector = new InventoryReportBuildDirector(dailyReportBuilder);
            reportDirector.BuildReport();

            var dailyReport = dailyReportBuilder.GetReport();
